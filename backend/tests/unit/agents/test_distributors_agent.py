...
def _crew_mock() -> MagicMock:
    draft = {"domain": _DOMAIN, "plan_id": "plan_001", "text": "Distributor analysis.",
...
async def test_returns_chapter_draft():
    ...
    with patch("agents.base_domain_agent.async_route", new=AsyncMock(return_value={"articles": []})), \
        ...
        draft = await DistributorsAgent().run(plan, "run_001")
...
async def test_fallback_on_crew_failure():
    ...
    with patch("agents.base_domain_agent.async_route", new=AsyncMock(return_value={"articles": [{"title": "t", "description": "d"}]})), \
